...
- Run `sudo killall usbfluxd` to stop any existing `usbfluxd` instances.
- Copy the command shown in the `Remote Debug` section and run it in your terminal. It should look something like this:
  ```bash
  sudo usbfluxd -n && usbfluxctl add [instance].smartdust.me:12345
  ```
